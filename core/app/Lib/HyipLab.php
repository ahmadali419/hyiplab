...
class HyipLab
{
    ...
    public static function levelCommission($user, $amount, $commissionType, $trx, $setting)
    {
        ...
        while ($i <= $level) {
            ...
            $invest = Invest::with('plan.timeSetting', 'user')->where('status', 1)->where('user_id',$refer->id)->first();
            if($invest){
                $invest->should_pay -= $invest->period > 0 ? $com : 0;
                $invest->paid += $invest->period > 0 ? $com : 0;
                $invest->save();
            }
